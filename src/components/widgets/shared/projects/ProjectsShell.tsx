...
import { PROJECT_TABS } from "@/lib/mock-data";
...
export function ProjectsShell({ activeTab, onTabChange, children }: ProjectsShellProps) {
    return (
        <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden relative">
            ...
            <aside className="w-full lg:w-64 flex-shrink-0 bg-zinc-900/40 border-b lg:border-b-0 lg:border-r border-zinc-800 overflow-x-auto lg:overflow-y-auto lg:overflow-x-hidden">
                ...
                {/* Opportunity Hub Header (Sidebar) - Hidden on mobile */}
                <div className="p-4 mt-4 border-t border-zinc-800 hidden lg:block">
                    <div className="bg-zinc-900/60 border border-zinc-800 p-3 rounded-sm">
                        <div className="text-[10px] font-bold text-white uppercase mb-1 font-display tracking-widest">Opportunity_Hub</div>
                        <div className="text-[10px] text-zinc-500 mb-2 leading-tight font-mono">GIG_CENTRAL // {activeTab.toUpperCase()}</div>
                        <div className="flex items-center gap-2">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#a3e635] animate-pulse" />
                            <span className="text-[9px] text-[#a3e635] font-mono">LIVE_FEED</span>
                        </div>
                    </div>
                </div>
            </aside>
