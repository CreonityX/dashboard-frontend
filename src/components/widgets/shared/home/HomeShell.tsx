...
export function HomeShell({ children }: HomeShellProps) {
    return (
        <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden relative">
            {/* Main Content Area */}
            <main className="flex-1 bg-zinc-900/40 overflow-y-auto custom-scrollbar relative backdrop-blur-md flex flex-col">
                {/* Content Background (Noise) */}
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.03] pointer-events-none z-0" />

                <div className="flex-1 p-6 lg:p-8 space-y-8 relative z-10 max-w-5xl mx-auto w-full">
                    {children}
                </div>
            </main>
